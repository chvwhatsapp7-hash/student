    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData['success'] == true) {
          ...
          final apiList = dataList.map((e) => Company(
            id: e['company_id'] ?? 0,
            name: e['name'] ?? '',
          ...
          setState(() {
            ...
            final staticNames = kCompanies.map((e) => e.name.toLowerCase()).toList();
            final apiNames = _apiCompanies.map((e) => e.name.toLowerCase()).toList();
            final missingInApi = kCompanies.where((c) => !apiNames.contains(c.name.toLowerCase())).toList();
            final missingInStatic = _apiCompanies.where((c) => !staticNames.contains(c.name.toLowerCase())).toList();
            debugPrint('Missing in API: ${missingInApi.map((e) => e.name).toList()}');
            debugPrint('Missing in static: ${missingInStatic.map((e) => e.name).toList()}');
          });
        } else {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            GestureDetector(
              onTap: () => setState(() { _search = ''; _filter = 'All'; }),
